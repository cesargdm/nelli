...
class WatsonViewController: UIViewController, BeaconDelegate, SpeechRecoginizerDelegate, TalkDelegate {
    ...
    func didFoundClosestBeacon(_ beacon: CLBeacon?) {
        ...
        guard let beacon = beacon else {
            ...
            if (watsonState == .idle) {
                setLabelText(nil, room: GO_CLOSER, alpha: 1)
            }
        ...
        if (watsonState == .idle) {
            setLabelText(piece.title, room: piece.room.stringValue, alpha: 1)
        }
    ...
    func setLabelText(_ text: String?, room: String?, alpha: Float?) {
